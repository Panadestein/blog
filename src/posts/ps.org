...
Combined with algorithmic improvements suggested by Marshall, based on a publication by Roger Hui[fn:2],
...
While the algorithms differ (see the next section), I argue that the C version's simple nested loops provide
a relevant baseline[fn:3]. Below, you will find the corresponding programs;
the C and Python versions implement a standard Sieve of Eratosthenes
without the more complex algorithmic optimizations found in the final BQN sieve[fn:4]:
...
[fn:3] Again, the C version shown provides a clear performance baseline. Achieving superior speed in C is
*undoubtedly possible*, but usually involves a steep increase in code complexity compared to this
straightforward implementation.
[fn:4] Sorry for the number of collapsibles; my focus for this blog is primarily on displaying BQN code.
