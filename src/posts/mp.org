...
But we're not finished yet. Here is a little divide-and-conquer (and cache-oblivious) [[https://en.wikipedia.org/wiki/Strassen_algorithm][algorithm]] in its classic
...
single-threaded BQN implementation[fn:3]. 

** Parallelism via MPI

To approach true bare-metal performance on par with BLAS/BLIS, we must leverage multiple cores.
As BQN lacked native support for SPMD programming, I developed and implemented bindings for a
useful subset of the Message Passing Interface (MPI), which are available on [[https://codeberg.org/panadestein/bqn-mpi][Codeberg]].
...
The implementation assumes a perfect square number of tasks, forming a processor grid of =⋈˜√p=,
and pads matrices whose dimensions are not divisible by =√p=.
...
[fn:3] For deeper insight into blocked matrix multiplication algorithms, I recommend
