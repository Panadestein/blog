...
at the time of writing, twice as fast as CBQN. It is possible to further improve =S4= though, as it was suggested
to me by Marshall Lochbaum, based on a publication by Roger Hui[fn:2]. The optimized code is located in the
=utils= namespace (referenced previously). Ultimately, this optimized BQN code proved twice as fast as a C baseline!
While the algorithms slightly differ, the C version's simple nested loops provide a relevant benchmark.
...
*Update*: after some discussion in the BQN Matrix room, dzaima has speed up CBQN's =0¨⌾(c⊸⊏)=, and now =utils.Sieve=
can compute the primes up to a billion in 1.2 seconds! See this other [[https://panadestein.github.io/blog/supp/pe/s.html][post]] for a comparison with Python and the straightforward
C implementation, both of them around 5 times slower!

#+begin_src bqn :tangle no :exports both
  7 utils.Sieve•_timed 1e9
#+end_src
